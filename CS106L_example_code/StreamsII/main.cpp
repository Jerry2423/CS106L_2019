...
int stringToInteger(const string& s) {
   ...
   int a;
   //iss >> a; //use istringstream to convert a string to an integer 
   //successful conversion: 1. head is valid; 2. no more thing at the end(e.g wrong: 34 dfsd)


   // look at the head
   if (/*iss.fail()*/ !(iss>>a)) {
       throw std::domain_error("error! no valid int at beginning");
   }
   char c;
   //iss >> c; // this return a stream that can be converted into a boolean value
   //look at the tail: if the input is valid, the position pointer will be at end, then if we try to read one more char, it will fail indicating this is successful and vise versa.
   //read fail indicates reading successful
   if (iss >> c) {
       throw std::domain_error("error! more than a single valid int");
   }
   // printStateBits(iss);
   return a;
}
...
void badWelcomeProgram() {
    ...
    //getline(cin, name); use getline can help
    cin >> name;
