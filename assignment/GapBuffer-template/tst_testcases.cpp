...
void TestCases::TEST3F_ostream_insertion_basic() {
   //  QSKIP("Remove this statement once you've templatized your part 3 code.");
    ...
    buf.move_cursor(-3);
    std::cout << "cursor index is " << buf.cursor_index() << std::endl;
...
void TestCases::TEST6E_copy_edge() {
    ...
    QVERIFY(buf2.size() == 1);
    buf3 = (buf4 = buf3) = buf4;
    // the original test case is wrong since the expression above would lead to undefined-behavior before c++ 17
    QVERIFY(buf3.size() == 1);
    QVERIFY(buf4.size() == 1);
